...
def load_checkpoint(
    ...
    # Handle dual projector parameters if they don't exist in the state dict
    if hasattr(model, 'logit_scale_caption') and 'logit_scale_caption' not in state_dict:
        logging.info('Initializing logit_scale_caption from logit_scale')
        state_dict['logit_scale_caption'] = state_dict['logit_scale'].clone()
        
    if hasattr(model, 'logit_bias_caption') and 'logit_bias_caption' not in state_dict and 'logit_bias' in state_dict:
        logging.info('Initializing logit_bias_caption from logit_bias')
        state_dict['logit_bias_caption'] = state_dict['logit_bias'].clone()
        
    # Initialize caption_proj if it exists in the model but not in the state dict
    if hasattr(model.visual, 'caption_proj') and 'visual.caption_proj' not in state_dict and 'visual.proj' in state_dict:
        logging.info('Initializing visual.caption_proj from visual.proj')
        state_dict['visual.caption_proj'] = state_dict['visual.proj'].clone()
    ...
    incompatible_keys = model.load_state_dict(state_dict, strict=False)  # Changed from strict to False
    
    if incompatible_keys.missing_keys:
        logging.warning(f"Missing keys in state_dict: {incompatible_keys.missing_keys}")
    if incompatible_keys.unexpected_keys:
        logging.warning(f"Unexpected keys in state_dict: {incompatible_keys.unexpected_keys}")
