...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        # Create scales for both taxonomic and caption paths
        self.logit_scale = nn.Parameter(torch.ones(lshape) * init_logit_scale)  # Keep for compatibility
        self.logit_scale_caption = nn.Parameter(torch.ones(lshape) * init_logit_scale)
        
        if init_logit_bias is not None:
            self.logit_bias = nn.Parameter(torch.ones(lshape) * init_logit_bias)  # Keep for compatibility
            self.logit_bias_caption = nn.Parameter(torch.ones(lshape) * init_logit_bias)
        else:
            self.logit_bias = None
            self.logit_bias_caption = None
    ...
    def get_logits(self, image, text, is_caption=False):
        tax_features, caption_features = self.encode_image(image, normalize=True)
        text_features = self.encode_text(text, normalize=True)
        
        # Use appropriate features and logit scale/bias based on label type
        if is_caption:
            image_features = caption_features
            logit_scale = self.logit_scale_caption
            logit_bias = self.logit_bias_caption
        else:
            image_features = tax_features
            logit_scale = self.logit_scale
            logit_bias = self.logit_bias
            
        image_logits = logit_scale.exp() * image_features @ text_features.T
    ...
    def get_all_logits(self, image, text):
        tax_features, caption_features = self.encode_image(image, normalize=True)
        text_features = self.encode_text(text, normalize=True)
        
        # Calculate both sets of logits
        tax_image_logits = self.logit_scale.exp() * tax_features @ text_features.T
        caption_image_logits = self.logit_scale_caption.exp() * caption_features @ text_features.T
        
        if self.logit_bias is not None:
            tax_image_logits += self.logit_bias
        if self.logit_bias_caption is not None:
            caption_image_logits += self.logit_bias_caption
            
        tax_text_logits = tax_image_logits.T
        caption_text_logits = caption_image_logits.T
        
        return {
            "tax_image_logits": tax_image_logits,
            "tax_text_logits": tax_text_logits,
            "caption_image_logits": caption_image_logits,
            "caption_text_logits": caption_text_logits
        }
    ...
    def forward_intermediates(
        ...
        if image is not None:
            ...
            # Handle both single and dual feature outputs from vision tower
            if "image_features_tax" in image_output and "image_features_caption" in image_output:
                # Dual feature case
                if normalize:
                    image_output["image_features_tax"] = F.normalize(image_output["image_features_tax"], dim=-1)
                    image_output["image_features_caption"] = F.normalize(image_output["image_features_caption"], dim=-1)
            elif "image_features" in image_output:
                # Original single feature case - handle for backward compatibility
                if normalize:
                    image_output["image_features"] = F.normalize(image_output["image_features"], dim=-1)
                # Add duplicate for caption path
                image_output["image_features_tax"] = image_output["image_features"]
                image_output["image_features_caption"] = image_output["image_features"]
        ...
        if output_logits and "text_features" in output:
            # Calculate taxonomic logits
            if "image_features_tax" in output:
                tax_image_logits = self.logit_scale.exp() * output["image_features_tax"] @ output["text_features"].T
                if self.logit_bias is not None:
                    tax_image_logits += self.logit_bias
                tax_text_logits = tax_image_logits.T
                output["tax_image_logits"] = tax_image_logits
                output["tax_text_logits"] = tax_text_logits
            
            # Calculate caption logits
            if "image_features_caption" in output:
                caption_image_logits = self.logit_scale_caption.exp() * output["image_features_caption"] @ output["text_features"].T
                if self.logit_bias_caption is not None:
                    caption_image_logits += self.logit_bias_caption
                caption_text_logits = caption_image_logits.T
                output["caption_image_logits"] = caption_image_logits
                output["caption_text_logits"] = caption_text_logits
                
            # For backward compatibility
            if "image_features" in output:
                image_logits = self.logit_scale.exp() * output["image_features"] @ output["text_features"].T
                if self.logit_bias is not None:
        ...
        if output_logit_scale_bias:
            ...
            output["logit_scale_caption"] = self.logit_scale_caption.exp()
            if self.logit_bias is not None:
                output['logit_bias'] = self.logit_bias
            if self.logit_bias_caption is not None:
                output['logit_bias_caption'] = self.logit_bias_caption
    ...
    def forward(
        ...
        if image is not None:
            ...
            tax_features, caption_features = None, None
            
        text_features = self.encode_text(text, normalize=True) if text is not None else None

        if self.output_dict:
            out_dict = {
                "image_features_tax": tax_features,
                "image_features_caption": caption_features,
                "text_features": text_features,
                "logit_scale": self.logit_scale.exp(),
                "logit_scale_caption": self.logit_scale_caption.exp()
            }
            if self.logit_bias is not None:
                out_dict['logit_bias'] = self.logit_bias
            if self.logit_bias_caption is not None:
                out_dict['logit_bias_caption'] = self.logit_bias_caption
            return out_dict

        # For backward compatibility with original implementation return format
        if self.logit_bias is not None:
            return tax_features, caption_features, text_features, self.logit_scale.exp(), self.logit_scale_caption.exp(), self.logit_bias, self.logit_bias_caption
        return tax_features, caption_features, text_features, self.logit_scale.exp(), self.logit_scale_caption.exp()
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(l):
        ...
        if isinstance(l, (CLIP, TextTransformer)):
            ...
            for attr_name in ["text_projection"]:
        ...
        if isinstance(l, VisionTransformer):
            ...
            for attr_name in ["proj", "proj_tax", "proj_caption"]:
                attr = getattr(l, attr_name, None)
                if attr is not None:
                    attr.data = attr.data.to(dtype)
