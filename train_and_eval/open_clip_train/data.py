...
def group_by_keys_nothrow(data, keys=base_plus_ext, lcase=True, suffixes=None, handler=None):
    ...
    for filesample in data:
        ...
        # 处理EOF标记（空字典）- 这是webdataset在shard结束时产生的
        if filesample == {}:
        ...
        if "fname" not in filesample or "data" not in filesample:
            if handler is not None:
                import logging
                logging.warning(f'Skipping invalid sample: {filesample.keys()}')
                continue
            else:
                raise KeyError(f'Missing required fields in sample: {filesample.keys()}')
