...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.text_type == 'random':
            images, sci, com, taxon, sci_com, taxon_com, caption = batch
            random.seed(step)
            # Adding an extra 'caption' to double its selection probability
            text_choices = ['sci', 'com', 'taxon', 'sci_com', 'taxon_com', 'caption']
            selected_idx = random.randrange(len(text_choices))
            selected_type = text_choices[selected_idx]
            texts = locals()[selected_type]  # Get the variable with the selected name
